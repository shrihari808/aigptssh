# aigptssh/api/dashboard/portfolio/portfolio_snapshot.py
import json
import os
import asyncio
...
async def get_portfolio_snapshot(portfolio: List[str] = Query(..., description="A list of stock tickers in the user's portfolio."), api_key: str = Depends(api_key_auth)):
    """
    Generates and streams a market snapshot for a given portfolio, providing progress updates.
    """
    async def stream_generator():
        # ... (steps 1 and 2 remain the same) ...
        yield create_progress_bar_string(5, f"Initializing for portfolio...").encode("utf-8")
        ...
        context_queries = {
            ...
            "indices_context": f"Provide a summary of the latest news for the stocks: {', '.join(portfolio)}.",
            "market_drivers_context": f"What were the key driving factors for the stocks in this portfolio: {', '.join(portfolio)}?"
        }
        ...
        llm_generator = PortfolioLLMGenerator(input_path=PORTFOLIO_DATA_JSON_PATH)
        portfolio_dashboard_content = await llm_generator.generate_dashboard_content()
        ...
        yield f"\n{json.dumps(portfolio_dashboard_content, indent=4)}".encode("utf-8")
