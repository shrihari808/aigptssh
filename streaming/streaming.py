...
from api.brave_searcher import BLACKLISTED_DOMAINS
...
async def web_rag_mix(
    ...
    async def tiered_stream_generator():
        ...
        final_response_text = ""
        with get_openai_callback() as cb:
            async for chunk in final_chain.astream({"context": final_context, "history": chat_history, "input": original_query, "today": today, "blacklist": BLACKLISTED_DOMAINS}):
                if chunk.content:
